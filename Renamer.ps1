...
Get-ChildItem -Path $PSScriptRoot -File -Recurse -exclude *.ps1 | % {
    ...
    if ($contents -match "ChargeApi") {
        $contents -replace 'ChargeApi', $apiName | Set-Content $_.PSPath
        Write-Host $("'{0}': contents changed." -f $fileName)
    }

    if ($fileName -match 'ChargeApi') {
        $newName = $_.Name -replace 'ChargeApi', $apiName
...
Where-Object { $_.Name -match 'ChargeApi' } | % {
    Write-Host $("Editing directory: '{0}'." -f $_.FullName)
    $newDirName = $_.Name -replace 'ChargeApi', $apiName
